  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
      <header className="sticky top-0 z-10 backdrop-blur bg-white/70 border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          <CupSoda className="w-6 h-6" />
          <h1 className="text-xl font-semibold">맛집 토너먼트 (Excel 기반)</h1>
          <Badge variant="secondary" className="ml-2">v2</Badge>

          <div className="ml-auto flex items-center gap-2">
            <Input
              placeholder="지역/제목 검색"
              value={query}
      ...
      <Dialog
        ...
        onOpenChange={(o) => setRewardDialog({ open: o })}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>참여해주셔서 감사합니다 🎉</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button onClick={() => setRewardDialog({ open: false })}>
              닫기
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          >
            ...
            >
              <CardHeader>
                ...
                {t.locationTag && (
                  <CardDescription>{t.locationTag}</CardDescription>
                )}
              </CardHeader>

              <CardContent>
                <div className="flex items-center justify-between text-sm text-slate-500">
                  <span>참가: {t.entries.length}개</span>
                  <span>{t.size}강</span>
                </div>
              </CardContent>
              ...
              <CardFooter>
                <div className="text-xs text-slate-400">
                  생성일: {new Date(t.createdAt).toLocaleDateString()}
                </div>
              </CardFooter>
            </Card>
          </motion.div>
...
function CreateTournament({
  ...
  const [rewardMode, setRewardMode] = useState<RewardMode>("random");
  const [rewardsPoolText, setRewardsPoolText] = useState(
    "배지: 맛집 큐레이터\n쿠폰: 아메리카노 무료\n랜덤 스티커 팩"
  );
  ...
  async function handleExcelUpload(e: any) {
    ...
    const rows = await parseExcelToEntries(file);
    setEntries(rows);
  }
  ...
  function handleCreate() {
    ...
    if (entries.length < 4)
      return alert("최소 4개 이상의 식당 데이터가 필요합니다.");
    ...
    const t = toTournament({
      ...
      entries,
      rewardMode,
      rewardsPool: rewardsPoolText.split("\n").map((s) => s.trim()),
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      <Card>
        ...
        <CardContent className="space-y-3">
          <Input
            placeholder="제목 (명동 맛집 32강)"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
          />
          <Input
            placeholder="지역 태그 (명동)"
            value={locationTag}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Excel 업로드</CardTitle>
          <CardDescription>명동 데이터(.xlsx)를 올려주세요</CardDescription>
        </CardHeader>
      ...
      <Card className="md:col-span-2">
        ...
        <CardContent className="grid md:grid-cols-3 gap-4">
          <Select
            value={rewardMode}
            onValueChange={(v: RewardMode) => setRewardMode(v)}
          >
            <SelectTrigger>
              <SelectValue placeholder="리워드 모드" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="random">랜덤</SelectItem>
              <SelectItem value="weighted">가중</SelectItem>
          ...
          <Textarea
            ...
            rows={6}
            value={rewardsPoolText}
            onChange={(e) => setRewardsPoolText(e.target.value)}
          />
        </CardContent>
...
function PlayTournament({
  ...
  data: Tournament;
  onUpdate: (t: Tournament, finishedReward?: { reward: string; code?: string }) => void;
  onBack: () => void;
}) {
  const [t, setT] = useState<Tournament>(data);
  ...
  function pickWinner(match: Match, choice: "a" | "b") {
    if (t.isFinished) return;
    if (!match.a || !match.b) return; // BYE 등
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <Badge variant="secondary">{t.rewardMode === "random" ? "랜덤" : "가중"} 리워드</Badge>
        {t.isFinished ? (
          <Badge className="ml-auto">우승: {t.winner?.name}</Badge>
        ) : (
          <span className="ml-auto text-sm text-slate-600">
            라운드 {t.currentRoundIndex + 1}/{totalRounds} · 진행률 {progress}%
          </span>
        )}
      </div>
...
function MatchCard({ match, onPick }: { match: Match; onPick: (m: Match, c: "a" | "b") => void }) {
  const disabled = !!match.winner || !match.a || !match.b || match.a.name === "BYE" || match.b.name === "BYE";
  ...
  return (
    <motion.div layout initial={{ opacity: 0.7, y: 6 }} animate={{ opacity: 1, y: 0 }}>
      <Card className={`overflow-hidden ${match.winner ? "ring-2 ring-emerald-400" : ""}`}>
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            <Swords className="w-4 h-4" /> {match.a?.name} <span className="text-xs text-slate-400">vs</span> {match.b?.name}
          </CardTitle>
          <CardDescription className="text-xs">
            {/* 간단 정보 */}
            {match.a?.rating ? `A 평점 ${match.a.rating}` : ""} {match.b?.rating ? ` · B 평점 ${match.b.rating}` : ""}
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="grid grid-cols-2 gap-2">
          <Button variant="secondary" disabled={disabled} onClick={() => onPick(match, "a")}>
            <div className="text-left">
              <div className="font-medium">{match.a?.name}</div>
              <div className="text-xs text-slate-500 line-clamp-2">{match.a?.review}</div>
            </div>
          </Button>

          <Button variant="secondary" disabled={disabled} onClick={() => onPick(match, "b")}>
            <div className="text-left">
              <div className="font-medium">{match.b?.name}</div>
              <div className="text-xs text-slate-500 line-clamp-2">{match.b?.review}</div>
            </div>
          </Button>
        </CardContent>
        ...
        {match.winner && (
          <CardFooter className="justify-between text-sm text-emerald-700">
            <span>선택됨: <b>{match.winner.name}</b></span>
            <Sparkles className="w-4 h-4" />
          </CardFooter>
        )}
      </Card>
...
function WinnerPanel({ winner }: { winner: Entry }) {
  return (
    <div className="p-6 rounded-3xl bg-gradient-to-br from-emerald-50 via-white to-emerald-100 border">
      <div className="text-center space-y-2">
        <Medal className="w-8 h-8 mx-auto" />
        <p className="text-2xl font-bold">우승: {winner.name}</p>
        {winner.rating && <p className="text-slate-600">평점 {winner.rating} · {winner.category}</p>}
        {winner.address && <p className="text-slate-500 text-sm">{winner.address}</p>}
        <p className="text-slate-600">참여해주셔서 감사합니다! 리워드 안내를 확인해주세요.</p>
      </div>
    </div>
  );
...
function resolveReward(t: Tournament): { reward: string; code?: string } {
  ...
  if (t.rewardMode === "weighted") {
    ...
    const chance = 0.35;
    if (Math.random() < chance) {
      return { reward: "쿠폰 지급", code: generateCoupon() };
    }
  } else {
    // 랜덤 모드: 낮은 확률
    const chance = 0.15;
